...
def singleBenders(d, cutViolationTolerance=1e-4, epsilon=1e-4):
    """
    ...
    while cutFound:
        ...
        # init singlecut constraint
        expr = 0

        # init Q value expectation
        exp_qvalue = 0
        ...
        for s in d.scenarios:
            ...
            # accumulate singlecut constraint
            ...
            # accumulate Q value expectation
            exp_qvalue += d.prob[s] * qvalue

        # check whether a violated Benders cut is found
        cutFound = tsol < exp_qvalue - cutViolationTolerance
        if cutFound:
            # update single-cut
            noCuts += 1
            MP.addConstr(t >= expr)
