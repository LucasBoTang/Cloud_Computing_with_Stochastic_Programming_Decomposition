...
def trustRegion(d, delta=15, mu=0.5, epsilon=1e-4):
    """
    Run trust region method
    """
    # build the master problem
    MP, xr, yr, t, trust_constrs = buildMP(d, delta)
    # build subproblems
    ...
    while True:
        ...
        xrsol_yt, yrsol_yt, m_yt, trust_constrs = \
        addTrustRegionAndSolveMP(MP, xr, yr, trust_constrs, xrsol_xt, yrsol_xt, delta, d)
        ...
        # convergence check
        if dec / (1 + abs(fx)) <= epsilon:
            print('The algorithm converges.')
            break

        # compute f(yt) and subgradient
        fy, expr = computeSubgradient(xrsol_yt, yrsol_yt, SP, xr, yr, xu, xo, yu, yo, d)
        # update best upper bound
        bestUB = min(fy, bestUB)
        print('Current f(y_t): {:.2f}'.format(fy))
...
def buildMP(d, delta):
    ...
    # init trust region
    exprs = [xr[u,v,p] <= delta for p in d.providers for v in d.VMs for u in d.users] + \
            [xr[u,v,p] >= - delta for p in d.providers for v in d.VMs for u in d.users] + \
            [yr[u,r] <= delta for r in d.routers for u in d.users] + \
            [yr[u,r] >= - delta for r in d.routers for u in d.users]
    trust_constrs = MP.addConstrs(e for e in exprs)

    return MP, xr, yr, t, trust_constrs
...
def addTrustRegionAndSolveMP(MP, xr, yr, trust_constrs, xrsol_xt, yrsol_xt, delta, d):
    """
    remove old trust region
    add new trust region
    solve master problem to get yt
    """
    # remove old trust region
    MP.remove(trust_constrs)

    # add new trust region
    exprs = [xr[u,v,p] - xrsol_xt[u,v,p] <= delta for p in d.providers for v in d.VMs for u in d.users] + \
            [xr[u,v,p] - xrsol_xt[u,v,p] >= - delta for p in d.providers for v in d.VMs for u in d.users] + \
            [yr[u,r] - yrsol_xt[u,r] <= delta for r in d.routers for u in d.users] + \
            [yr[u,r] - yrsol_xt[u,r] >= - delta for r in d.routers for u in d.users]
    trust_constrs = MP.addConstrs(e for e in exprs)
    ...
    return xrsol_t, yrsol_t, MPobj, trust_constrs
